...
#################################################
NDK_R25B_PATHS=(
            /home/pub/ndk/android-ndk-r25b/
            ~/work/android/ndk/android-ndk-r25b/
            )
NDK_R17C_PATHS=(
            /home/pub/ndk/android-ndk-r17c/
            ~/work/android/ndk/android-ndk-r17c/
            )
NDK_R16B_PATHS=(
...
# Search r25b ndk # NOTE: r25b use ndk toolchain file
if [ "${FOUND_NDK}" = "0" ]; then

    for NDK_PATH in ${NDK_R25B_PATHS[@]};
    do
        if [ -d ${NDK_PATH} ]; then
            FOUND_NDK=1
...
# Search r17c ndk # NOTE: r17c use ndk toolchain file
if [ "${FOUND_NDK}" = "0" ]; then

    for NDK_PATH in ${NDK_R17C_PATHS[@]};
    do
        if [ -d ${NDK_PATH} ]; then
            FOUND_NDK=1
            ANDROID_NDK=${NDK_PATH}
...
if [ $NDK_VERSION -eq 0 ]; then
    ...
    if [ $NDK_VERSION -ge 25 ]; then
        TOOLCHAIN_FILE=${ANDROID_NDK}/build/cmake/android.toolchain.cmake
        NATIVE_API_LEVEL="android-28"
    elif [ $NDK_VERSION -ge 17 ]; then
        TOOLCHAIN_FILE=${ANDROID_NDK}/build/cmake/android.toolchain.cmake
        NATIVE_API_LEVEL="android-28"
    elif [ $NDK_VERSION -ge 16 ]; then
        TOOLCHAIN_FILE=${ANDROID_NDK}/build/cmake/android.toolchain.cmake
